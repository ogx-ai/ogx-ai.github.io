...
ogx supports multiple search modes beyond basic vector similarity:
...
For hybrid search, ogx offers configurable ranking strategies:
...
**Note**: All ogx-specific parameters (`provider_id`, `embedding_model`, and `embedding_dimension`) must be specified in the `extra_body` parameter when creating vector stores.
...
**Note**: While ogx is OpenAI-compatible, it also supports additional options beyond the standard OpenAI API. When creating vector stores, you can specify custom embedding models and embedding dimensions that will be used when processing chunks from attached files.
