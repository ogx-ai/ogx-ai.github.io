...
description: Complete guide to using ogx server and client SDK to build AI agents
...
In this guide, we'll walk through how you can use the ogx (server and client SDK) to test a simple agent.
A ogx agent is a simple integrated system that can perform tasks by combining a Llama model for reasoning with
tools (e.g., RAG, web search, code execution, etc.) for taking actions.
In ogx, we provide a server exposing multiple APIs. These APIs are backed by implementations from different providers.

ogx is a stateful service with REST APIs to support seamless transition of AI applications across different environments. The server can be run in a variety of ways, including as a standalone binary, Docker container, or hosted service. You can build and test using a local server first and deploy to a hosted endpoint for production.

In this guide, we'll walk through how to build a RAG agent locally using ogx with [Ollama](https://ollama.com/)
...
```
### Step 2:  Run ogx
ogx is a server that exposes multiple APIs, you connect with it using the ogx client SDK.

<Tabs>
<TabItem value="venv" label="Using venv">
You can use Python to install dependencies and run the ogx server, which is useful for testing and development.

ogx uses a [YAML configuration file](../distributions/configuration) to specify the stack setup,
...
Now let's install dependencies and run the ogx config for Ollama.
...
You can use a container image to run the ogx server. We provide several container images for the server
...
Now you can use the ogx client to run inference and build agents!

You can reuse the server setup or use the [ogx Client](https://github.com/meta-llama/llama-stack-client-python/).
...
Now you can run inference using the ogx client SDK.
...
Next we can move beyond simple inference and build an agent that can perform tasks using the ogx server.
#### i. Create the Script
...
Congrats! 🥳 Now you're ready to [build your own ogx applications](../building_applications/)! 🚀
