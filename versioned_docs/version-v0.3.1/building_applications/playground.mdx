...
description: Interactive interface to explore and experiment with ogx capabilities
...
The ogx Playground is currently experimental and subject to change. We welcome feedback and contributions to help improve it.
:::

The ogx Playground is a simple interface that aims to:
- **Showcase capabilities and concepts** of ogx in an interactive environment
- **Demo end-to-end application code** to help users get started building their own applications
- **Provide a UI** to help users inspect and understand ogx API providers and resources

## Key Features

### Interactive Playground Pages

The playground provides interactive pages for users to explore ogx API capabilities:
...
- Uses ogx's `/agents` API to create and manage RAG sessions
...
- Uses ogx's `/scoring` API for flexible evaluation workflows
...
- Uses ogx's `/eval` API for comprehensive evaluation
...
**Provider Management**
- Inspect available ogx API providers
...
- Inspect ogx API resources including:
...
The playground works with any ogx distribution. Popular options include:
...
- **Learning ogx**: Hands-on exploration of API capabilities
...
- **Demo Creation**: Showcasing ogx capabilities to stakeholders
...
- **[Core Concepts](/docs/concepts)** - Understanding ogx fundamentals
