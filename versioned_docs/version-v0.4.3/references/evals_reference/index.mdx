...
This first example walks you through how to evaluate a model candidate served by ogx on open benchmarks. We will use the following benchmark:
...
- In this example, we will demonstrate how to evaluate a agent candidate served by ogx via `/agent` API.
...
- `Dataset` used for evaluation:
  ...
  - (2) `--dataset-id`: pre-registered dataset in ogx
