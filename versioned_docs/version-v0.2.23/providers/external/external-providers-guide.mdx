...
Additionally you can configure the `external_providers_dir` in your ogx configuration. This method is in the process of being deprecated in favor of the `module` method. If using this method, the external provider directory should contain your external provider specifications:
...
5. Configure ogx to use external providers:

```yaml
external_providers_dir: ~/.llama/providers.d/
```

The provider will now be available in ogx with the type `remote::custom_ollama`.
...
To install ogx with this external provider a user can provider the following build.yaml:
