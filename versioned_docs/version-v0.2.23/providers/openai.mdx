...
ogx exposes an OpenAI-compatible API endpoint at `/v1/openai/v1`. So, for a ogx server running locally on port `8321`, the full url to the OpenAI-compatible API endpoint is `http://localhost:8321/v1/openai/v1`.
...
When using an OpenAI client, set the `base_url` to the `/v1/openai/v1` path on your ogx server.
